...
const WizardStepSetup: React.FC<WizardStepSetupProps> = ({ titleError, onTitleErrorClear }) => {
	const {
		tableTitle,
		setTitle,
		source,
	...
	return (
		<div className="w-full max-w-2xl mx-auto space-y-8">
			<h1 className="text-center text-2xl font-bold text-gray-800 my-4 pb-2">
			...
			<div className="space-y-2">
				...
				<input
					...
					className={`w-full h-12 px-4 text-base border rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 ${
						titleError
								...
								<Alert variant="warning" className="bg-amber-50 border-amber-200">
									...
									<AlertDescription className="text-amber-800">
										...
										<p className="text-sm mt-2 m-0">
											{__(
												'Your WooCommerce store appears to be empty. Add some products to create a table.',
												'productbay'
											)}
										</p>
										<a
											target="_blank"
											href={`${(window as any).ajaxurl?.split(
												'admin-ajax.php'
							...
							{!isStatsLoading && currentStats?.products === 0 && (
								<Alert variant="warning" className="bg-amber-50 border-amber-200">
									...
									<AlertDescription className="text-amber-800">
										...
										<p className="text-sm mt-2 m-0">
											{__(
												'Set sale prices on some products to use this source type.',
												'productbay'
											)}
										</p>
										<a
