...
export const BulkSelectConfig = ({ value, onChange }: BulkSelectConfigProps) => {
	...
	return (
		<div className="bg-white p-4 rounded-lg border hover:bg-orange-50 border-gray-200 mb-8">
			<h3 className="text-sm font-semibold text-gray-900 mb-4 pb-1 border-b border-gray-300 pb-4">
				{__('Bulk Actions & Selection', 'productbay')}
			</h3>
			...
			<SettingsOption
				...
				description={__(
					'Adds a checkbox column and an "Add to Cart" button for bulk actions.',
					'productbay'
				)}
				...
				<div className="mt-4 pt-4 border-t border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-6">
					...
					<div>
						...
						<Select
							...
							onChange={(val) =>
								handleChange({
									position: val as 'first' | 'last',
								})
							}
							options={[
								{
									value: 'first',
									label: __('First Column (Left)', 'productbay'),
								},
								{
									value: 'last',
									label: __('Last Column (Right)', 'productbay'),
								},
							]}
							size="xs"
						/>
					...
					<div>
						...
						<Select
							...
							onChange={(val) =>
								handleChange({
									visibility: val as VisibilityMode,
								})
							}
