	...
	return (
		<div className="space-y-6">
			...
			<StepHeading title="Source:">
				<span
					className={ `inline-flex items-center rounded-full font-medium border px-2 py-1 justify-center text-sm ${
						isSourceComplete
					...
					>
						{ Object.keys( categories ).map( ( type ) => (
							<button
								key={ type }
								onClick={ () =>
									setTableData( { source_type: type } )
								}
								className={ `relative bg-white py-2 px-4 text-sm font-medium capitalize ${
									source_type === type
							...
							>
								...
								<AlertDescription className="text-amber-800">
									...
									<p className="text-sm mt-2 m-0">
										Your WooCommerce store appears to be
										empty. You need at least one product to
									...
									<a
										...
										href={ `${ (
											...
											'admin-ajax.php'
										)[ 0 ] }post-new.php?post_type=product` }
						...
						sourceStats[ 'sale' ]?.products === 0 && (
							...
							>
								...
								<AlertDescription className="text-amber-800">
									...
									<p className="text-sm mt-2 m-0">
										There are no products with a sale price
										set in your store.
									</p>
									<a
										target="_blank"
