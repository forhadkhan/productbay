	...
	const getButtonVariant = (
		...
		switch (variant) {
			...
			case 'success':
				return 'success';
			case 'secondary':
				return 'outline'; // Mapping secondary to outline for better contrast in new design
			default:
				return 'default';
		}
	};
		...
		>
			...
			{closeOnEsc && !fullScreen && (
				<p className="fixed bottom-4 left-1/2 -translate-x-1/2 text-sm text-white/70 bg-black px-4 py-1 rounded-full drop-shadow-lg flex items-center gap-2 pointer-events-none">
					{__('To Close, Press', 'productbay')}{' '}
					<kbd className="mx-1 px-1.5 rounded-full text-xs bg-white text-black font-mono">
						Esc
					</kbd>
				</p>
			)}
