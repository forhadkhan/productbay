...
export const ColorPicker: React.FC<ColorPickerProps> = ({
	...
	const hsv = useMemo(() => {
		...
		const derivedHsv = rgbToHsv(isNaN(r) ? 0 : r, isNaN(g) ? 0 : g, isNaN(b) ? 0 : b);
	...
	const displayHex = useMemo(() => {
		...
		const b = rgba.b ? parseInt(rgba.b, 10) : 0;
		const a = rgba.a ? parseFloat(rgba.a) : 1;
		return rgbaToHex(isNaN(r) ? 0 : r, isNaN(g) ? 0 : g, isNaN(b) ? 0 : b, isNaN(a) ? 1 : a);
		...
		<div className={cn('relative inline-block', className)} ref={containerRef}>
			...
			<div className="flex items-center gap-2">
				...
				>
					...
					{(triggerMode === 'icon' || triggerMode === 'both') &&
						(isOpen ? (
							<span title={__('Close', 'productbay')}>
								<X className="w-3.5 h-3.5" />
							</span>
						) : (
							<span title={__('Edit Color', 'productbay')}>
								<Pencil className="w-3.5 h-3.5" />
							</span>
						))}
					{/* Toggle text between Edit Color (closed) and Close (open) */}
					{(triggerMode === 'text' || triggerMode === 'both') && (
						<span>
							{isOpen ? __('Close', 'productbay') : __('Edit Color', 'productbay')}
						</span>
					)}
				</button>
			</div>
				...
				>
					...
					<div className="px-1">
						...
						{activeFormat === 'hex' ? (
							<div className="flex flex-col gap-1">
								...
								/>
								<span className="text-[9px] uppercase text-gray-400 font-bold text-center">
									Hex
								</span>
							</div>
						) : activeFormat === 'rgb' ? (
								...
								{(['r', 'g', 'b', 'a'] as const).map((key) => (
									<div key={key} className="flex flex-col gap-1 text-center">
										<input
											...
											value={rgba[key]}
											onChange={(e) =>
												handleComponentChange('rgb', key, e.target.value)
											}
								...
								{(['h', 's', 'l', 'a'] as const).map((key) => (
									<div key={key} className="flex flex-col gap-1 text-center">
										<input
											...
											value={hsla[key]}
											onChange={(e) =>
												handleComponentChange('hsl', key, e.target.value)
											}
											className="w-full px-1 py-1.5 text-xs border border-gray-200 rounded focus:border-blue-400 focus:outline-none transition-colors font-mono text-center"
										/>
										<span className="text-[9px] uppercase text-gray-400 font-bold">
					...
					{showPresets && presets.length > 0 && (
						<div className="border-t border-gray-100 mt-3 pt-3">
							<span className="text-[10px] uppercase text-gray-400 font-bold ml-1 mb-2 block">
								{__('Presets', 'productbay')}
							</span>
