...
- **[Activity Logging](./LOGGING.md)** - File-based audit system with context tracking and live monitoring
...
## Extensibility API (v1.0.1+)

ProductBay uses a **WordPress hook-based add-on architecture** to support pro features without modifying core code.

### Architecture

```
productbay (free)                    productbay-pro (add-on)
├── do_action / apply_filters  ←→   ├── add_action / add_filter
├── No direct DB in pro             └── Extends via hooks only
```
...
| Layer | File(s) | Hooks |
|-------|---------|-------|
| Core | `Plugin.php` | `productbay_loaded`, `productbay_admin_init` |
| Data | `TableRepository.php` | `productbay_before_save_table`, `productbay_after_save_table`, `productbay_after_delete_table`, `productbay_table_data` |
| API | `Router.php`, `SettingsController.php`, `SystemController.php` | `productbay_register_routes`, `productbay_default_settings`, `productbay_get_settings`, `productbay_settings_updated`, `productbay_system_status` |
| Frontend | `TableRenderer.php` | `productbay_query_args`, `productbay_table_columns`, `productbay_cell_output`, `productbay_table_output`, `productbay_table_styles`, `productbay_before_table`, `productbay_after_table`, `productbay_before_row`, `productbay_after_row`, `productbay_toolbar_start`, `productbay_toolbar_end` |
| AJAX | `AjaxRenderer.php` | `productbay_ajax_filter_response`, `productbay_after_bulk_add_to_cart` |
| Shortcode | `Shortcode.php` | `productbay_shortcode_atts`, `productbay_enqueue_frontend_assets` |
| Admin | `Admin.php` | `productbay_after_register_menu`, `productbay_admin_script_data`, `productbay_enqueue_admin_assets` |

> Full reference: [Hooks & Filters Documentation](../docs/developer/hooks.md)
